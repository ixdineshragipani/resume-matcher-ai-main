...
import { 
  Sparkles, ArrowLeft, Users, Search, Filter, 
  FileText, Clock, UserCheck, Calendar, TrendingUp
} from 'lucide-react';
...
const JobApplicants = () => {
  ...
  // Get job details and applicants from localStorage
  const [jobData, setJobData] = useState(() => {
    const jobs = JSON.parse(localStorage.getItem('companyJobs') || '[]');
    return jobs.find(j => j.id === parseInt(jobId)) || null;
  });
  ...
  const updateApplicantStatus = (applicantId, newStatus) => {
    ...
    // Update local state
    setJobData(updatedJobs.find(j => j.id === parseInt(jobId)));
  };
  ...
  const stats = [
    { label: 'Total Applicants', value: applicants.length, icon: Users, color: 'primary' },
    { label: 'Shortlisted', value: applicants.filter(a => a.status === 'shortlisted').length, icon: UserCheck, color: 'success' },
    { label: 'Interviews', value: applicants.filter(a => a.status === 'interview').length, icon: Calendar, color: 'warning' },
    { label: 'Avg ATS Score', value: applicants.length ? Math.round(applicants.reduce((sum, a) => sum + a.atsScore, 0) / applicants.length) : 0, icon: TrendingUp, color: 'accent' },
  ];
              ...
              >
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-center">
                      <div className={`ats-score ${getAtsScoreClass(applicant.atsScore)}`}>
                        {applicant.atsScore}
                      </div>
                    ...
                    <div className="text-center">
                      <span className={`status-badge capitalize ${getStatusBadge(applicant.status)}`}>
                        {applicant.status}
                      </span>
                    </div>
