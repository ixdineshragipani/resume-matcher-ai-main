...
import { 
  Sparkles, LogOut, Briefcase, MapPin, DollarSign, 
  Calendar, Search, Plus, ChevronRight, Users, Clock
} from 'lucide-react';
...
const CompanyDashboard = () => {
  ...
  const shortlistedCount = jobs.reduce((sum, job) => 
    sum + (job.applicants?.filter(a => a.status === 'shortlisted').length || 0), 0
  );

  const stats = [
    { label: 'Active Jobs', value: activeJobs, icon: Briefcase, color: 'primary' },
    { label: 'Total Applicants', value: totalApplicants, icon: Users, color: 'accent' },
    { label: 'Shortlisted', value: shortlistedCount, icon: Users, color: 'success' },
    { label: 'This Month', value: jobs.filter(j => {
      const jobDate = new Date(j.postedDate);
      const now = new Date();
      return jobDate.getMonth() === now.getMonth() && jobDate.getFullYear() === now.getFullYear();
    }).length, icon: Calendar, color: 'warning' },
  ];
          ...
          <div className="space-y-4">
            {filteredJobs.map((job, index) => (
              <div
                ...
                className={`card-interactive p-6 cursor-pointer animate-slide-up delay-${Math.min((index + 4) * 100, 500)}`}
              >
